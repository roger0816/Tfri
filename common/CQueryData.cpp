...
CQueryData::CQueryData(QObject *parent) : QObject(parent)
{
    ...
    connect(&m_timer,&QTimer::timeout,this,&CQueryData::query);
...
void CQueryData::setFileList(QString sUser,QString sGroup, QStringList listFile)
{
    for(int i=0;i<listFile.length();i++)
    {
        ...
        QFile file(listFile.at(i));

        if(file.open(QIODevice::ReadOnly))
        {
            QByteArray dData = file.readAll().toBase64();
...
void CQueryData::query()
{
    ...
    {
        //   qDebug()<<"no query data";
        return ;
    }

    if(m_bBusyLock)
    {
        m_iBusyTime+=QUERY_MSEC;

        if(m_iBusyTime>= 1000*60*2)
        {
            m_iBusyTime = 0;
    ...
    CPicData cData = m_listData.first();
    ...
    //    sUserId = m_listData.first().sUser;

    //    sImagePath = m_listData.first();
    ...
    //        if(m_listUserId.length()>0)
    //            m_listUserId.pop_front();
  ...
  //  QString sUrl = QString("http://161.35.98.42:8080/v2.1/classify_image");
      ...
      //  QString sUrl = QString("http://tfri.chencan.info:8080/v2.1/classify_image?decimalplaces=3");

    QString sUrl = QString("http://tfri2.chencan.info:8080/v2.1/classify_image?decimalplaces=3");
    ...
    //      upng = upng_new_from_file(sFileName.toStdString().c_str());
    //      if (upng != NULL) {
    //        upng_decode(upng);
    ...
    QByteArray dTmpPic = QByteArray::fromBase64(dPic);
    ...
    image_part.setHeader(QNetworkRequest::ContentLengthHeader,dTmpPic.size());
    //image_part.setBodyDevice(file);
    image_part.setBody(dTmpPic);
    ...
    //        reply->setProperty("user",sUser);

    //        reply->setProperty("file_name",sFileName);
...
void CQueryData::slotFinish(QNetworkReply *reply)
{
    ...
    //    QString sUser = reply->property("user").toString();

    //    QString sFileName =reply->property("file_name").toString();



    CPicData cPic = m_listData.first();
